 ...
 *
 * <p>The input is expected to be a {@link KV} where the key is a string identifier that
 * suppression is desired for, and the value is any alerts associated with this identifier.
 * State is used within the applicable window for the event.
 *
 * <p>If no alert has been seen for a given identifier, the earliest alert for the identifier
 * is submitted based on the alert timestamp. If the identifier has already been seen in-window,
 * a notice is output in the logs and the alerts are dropped.
 */
