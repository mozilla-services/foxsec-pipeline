...
public class Amo implements Serializable {
  ...
  public static PCollection<Alert> executePipeline(
    ...
    if (options.getGenerateConfigurationTicksInterval() > 0) {
      EventFilter filterForCfgTicks = new EventFilter();
      filterForCfgTicks.addRule(new EventFilterRule().wantSubtype(Payload.PayloadType.CFGTICK));

      PCollection<Event> parsedCfgTicks =
          input.apply(
              "cfgtick-parser",
              ParDo.of(
                  new ParserDoFn()
                      .withConfiguration(cfg)
                      .withInlineEventFilter(filterForCfgTicks)));
